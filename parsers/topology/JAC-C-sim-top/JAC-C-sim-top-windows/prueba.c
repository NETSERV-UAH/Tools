...
int main() {
	...
	fp = _popen(comando_string, "r");                  //Abre pipe para obetener como input el output del prompt al comando DIR dentro del
	if (fp == NULL)                                    //   directorio raiz.
		exit(-1);
	...
	while (fgets(path, PATH_MAX, fp) != NULL) {
		...
		if (z == 1) {
			...
			ListarDir(path, name_dir_dinamica[i]);      //Discrimina los "Directorios" : <DIR> .\n     y  <DIR> ..\n
			i++;
		}
	}
...
int NumTopos(char * name_dir) {
	...
	return num_topo - 2 ;          //Se le resta dos por los dos dir " . " y " . . "

}

int contenidoString(char * path) {
	int i, z = 0;
...
int Parser(char * nombre_topo, char * nombre_directorio_raiz) {
	...
	nombre_topo[strlen(nombre_topo) - 1] = '\0';
	strcpy(ruta_archivo_aparsear,"C:.\\");
	...
	strcpy(ruta_archivo_parseado, "C:.\\");
