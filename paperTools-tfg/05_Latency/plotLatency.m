...
values = mytable{:, :};
mytableB = readtable('Latency_XDP.csv');
valuesXDP = mytableB{:, :};
mytableC = readtable('Latency_P4.csv');
valuesP4 = mytableC{:, :};
...
XDP_std = std(valuesXDP);
P4_std = std(valuesP4);
std_all = [XDP_std; P4_std];
...
%    %  if i == 6
%    %    text(hB(i).XData+hB(i).XOffset,hB(i).YData+0.2,string(roundn(hB(i).YData,-2)),'VerticalAlignment','bottom','horizontalalign','center');  
%    %  end
%    text(hB(i).XData+hB(i).XOffset-0.095,hB(i).YData+0.2,string(roundn(hB(i).YData,-2)),'VerticalAlignment','bottom','horizontalalign','center');
%  end
...
for i = 1:nbars
    ...
    errorbar(x, values(:,i),std_all(:,i), 'k', 'linestyle', 'none');
end
